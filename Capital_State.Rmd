...
```{r}
tmp.paths <- read.csv("./Predicate_paths/capitals_3.csv", header = T)
tmp.paths$X <- NULL
...
#as.character(features_captial_state$path)
...
```
```{r}
wekares<-eval.df(tmp.paths[c("label", cutoff.k(weights_capital_state, 6))])
wekares[["eval"]]
```
...
clusterExport(cl = cl, varlist=c("full_predicated_path", "weights_capital_state", "eval.df", "cutoff.k", "fm_val","str_split",
                               "as.numeric"), envir = environment())
res_AOROC <- rbind.fill(parLapply(cl, seq(1,13,by = 1), function(x){
library(FSelector)
library(RWeka)
...
```
```{r}
wekares<-eval.df(full_predicated_path[,c("label", row.names(weights_capital_state)[c(1,2,3,4,5,6,8,11,12,13,24)])])
wekares[["eval"]]
```


```{r}
wekares<-eval.df(full_predicated_path[,c("label", row.names(weights_capital_state)[c(1,2,3,4,5,6,8,11,12,13)])])
wekares[["eval"]]
```
...
```{r}
full_predicated_path <- res_capital_state[["raw"]]
wekares<-eval.df(full_predicated_path[,c("label", cutoff.k(weights_capital_state, 20))])
wekares[["eval"]]
```
...
```
```{r}
wekares<-eval.df(full_predicated_path[,c("label", cutoff.k(weights_capital_state, 15))])
wekares[["eval"]]
```
